...
var translate = (function () {
          ...
          if ((typeof val1 === "undefined" ? "undefined" : _typeof(val1)) !== "object" || !val1 || val1.play !== true) {
            ...
            val2.forEach(function (element, index) {
              if (element.key && element.val) {
                //first-tier keys are 'button', 'score', etc
  ...
  function unwrap(element, val1) {
    //assumes .key and .val have been checked
    val1[element.key] = val1[element.key] || {}; //make sure it exists
    element.val.forEach(function (el2, ind) {
      //go through the array of key/value pairs
      if (el2.key && el2.val) {
        if (el2.val instanceof Array && el2.val.length && el2.val[0].key) {
          //another layer down
